...
<Tabs syncKey="lang">
  ...
  <TabItem label="C#">
    ```csharp
    s.DefineState(() => VMap(("total", Value.Of(0))));
  ...
  <TabItem label="Go">
    ```go
    s.Stimulus("I add {int}", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
    	// FULL-REPLACEMENT state: return the whole next state.
    	m := smap(state)
    	m["total"] = varar.IntValue(asInt(m["total"]) + asInt(args[0]))
    	return varar.Returns(varar.MapValue(m))
    })
    s.Sensor("the total is {int}", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
    	return varar.Returns(smap(state)["total"])
    })
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Go">
    ```go
    s.Sensor("the alarm fired", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
    	if b, _ := smap(state)["alarm"].AsBool(); !b {
    		return varar.Fails("no alarm")
    	}
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Java">
    ```java
    StateBinder<Ctx> s = registrar.steps(() -> new Ctx(List.of(5, 6)));
    s.param(
            "numbers",
            Pattern.compile("\\d+(?:, \\d+)*"),
  ...
  <TabItem label="C#">
    ```csharp
    s.DefineState(() => VMap(("dice", Value.List([Value.Of(5), Value.Of(6)]))));
    s.Param("numbers", @"\d+(?:, \d+)*",
  ...
  <TabItem label="Go">
    ...
    // "The dice show 5, 6" — {numbers} transforms to [5, 6]
    s.Sensor("The dice show {numbers}", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
    	return varar.Returns(smap(state)["dice"])
    })
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Go">
    ```go
    s.Sensor("I should have {int} cukes in my {word} belly", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
    	m := smap(state)
    	return varar.Returns(varar.ListValue(m["count"], m["bellyName"]))
    })
    ```
  </TabItem>
</Tabs>
