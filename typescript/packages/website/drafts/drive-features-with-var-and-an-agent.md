...
description: The per-feature loop once your agent is wired up — talk in customer language, let the agent specify, then iterate on the oath.
area: guides
order: 2
---
...
- Agent instructions in place — the agent must already know to write oaths first.
...
### 2. Let the agent write the oath
...
If the oath doesn't match what you meant, push back now, not later. "The oath doesn't say what happens when the name is whitespace-only" is a much cheaper conversation than "the code is wrong in production".
...
- Is the agent editing the oath to make the failure go away? Stop it. That breaks the contract.
- Is the agent silently changing other oaths that previously passed? Stop it. Ask why.

### 4. Iterate on the oath, not the code

When the agent shows you a green suite, your review is of the oath, not the diff. Ask:

- Are there examples we're missing? Edge cases? Error paths?
- Does the oath say what we actually want, or did we settle?

Add the missing examples. Let the agent fill in the implementation. The cycle repeats until the oath is honest and the suite is green.

### 5. Commit when the oath is honest

A commit means: the oath captures what we want, and the suite proves the code satisfies it. The commit message describes the *behaviour* added, not the implementation:
...
- The agent is rewriting large amounts of unrelated code to pass one new example. Your oath is probably too coupled to internals — back out and rewrite it in higher-level language.
...
- The suite is green but you can't tell from the oath what the system does. Your oath is incomplete or written for the machine, not for a human reader.

In all three cases the oath is the problem, not the agent. Fix it there.
...
- **Don't** let the loop run unattended on something risky. The agent will satisfy *what you said*, not *what you meant*. Risky surface area still wants a human on the oath.
