...
export function vararVitestPlugin(options: VararVitestPluginOptions = {}): Plugin {
  ...
  return {
    name: '@varar/vitest',
    async config() {
      // varar.config.json is the single source of truth for which files are oaths.
...
// point at the right oath line — with a string-literal test name — without
// any source map.
