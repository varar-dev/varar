...
test('stepAt resolves the step from a .bdd.md match and returns every matched site with values', async () => {
  const { dir, cleanup } = tempWorkspace((dir) => {
    writeFileSync(join(dir, 'bdd.config.ts'), 'export default {}\n')
    writeFileSync(
      join(dir, 'a.steps.ts'),
...
`,
  ...
  try {
    ...
    // of the .bdd.md "Given I greet \"world\"" sentence.
    const result = h.stepAt({
      uri: `file://${join(dir, 'a.bdd.md')}`,
      position: { line: 2, character: 11 },
    })
...
test('renameStep from a .bdd.md uses CucumberExpressionGenerator on the new sentence', async () => {
  const { dir, cleanup } = tempWorkspace((dir) => {
    writeFileSync(join(dir, 'bdd.config.ts'), 'export default {}\n')
    writeFileSync(
      join(dir, 'a.steps.ts'),
...
`,
  ...
  try {
    ...
    await store.reindex(dir)
    const h = buildHandlers(store)
    // The user F2'd in a.bdd.md and typed a new sentence.
    const result = h.renameStep({
      uri: `file://${join(dir, 'a.bdd.md')}`,
      position: { line: 2, character: 11 },
...
test('completions: returns nothing for non-.bdd.md files', async () => {
  const { dir, cleanup } = tempWorkspace((dir) => {
    writeFileSync(join(dir, 'bdd.config.ts'), 'export default {}\n')
    writeFileSync(
      join(dir, 'a.steps.ts'),
