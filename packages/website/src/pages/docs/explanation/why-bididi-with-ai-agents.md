...
title: Why bididi pairs well with AI coding agents
...
# Why bididi pairs well with AI coding agents
...
Once your acceptance criteria are precise enough to execute, an agent can use them as a fitness function. It writes code, runs the bididi suite, reads the failures, iterates. The loop closes itself. You don't have to babysit each diff because the criteria already encode what "done" means.

This is the loop that works:

1. Write the example in plain language as a bididi spec.
2. Hand the spec to the agent.
3. The agent writes step definitions and production code.
4. Bididi runs. Failures come back as feedback.
...
- **Outer loop** — bididi specs describe the behaviour the system should exhibit. The agent (or you) writes them with the customer in mind.
