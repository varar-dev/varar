...
public sealed record Globs(ImmutableArray<string> Include, ImmutableArray<string> Exclude);
...
public static class ConfigFile
{
    ...
    public static readonly ParsedConfig Empty = new(
        new Globs([], []),
        [],
        ImmutableDictionary<string, string>.Empty);

    /// <summary>Parses config text (no filesystem). Fails loudly with the path prefixed.</summary>
    public static ParsedConfig Parse(string jsonText, string sourcePath)
    {
        JsonDocument doc;
        try
        {
        ...
        {
            throw new ConfigException($"{sourcePath}: invalid JSON: {e.Message}");
        }
        ...
        using (doc)
        {
            ...
            {
                throw new ConfigException($"{sourcePath}: top level must be an object");
            }
            ...
            if (root.TryGetProperty("docs", out var docsEl) && docsEl.ValueKind != JsonValueKind.Null)
            {
                ...
                {
                    throw new ConfigException($"{sourcePath}: \"docs\" must be an object");
                }
                ...
                foreach (var prop in docsEl.EnumerateObject())
                {
                    ...
                    {
                        throw new ConfigException($"{sourcePath}: unknown key \"docs.{prop.Name}\" (known: include, exclude)");
                    }
                }
            ...
            if (root.TryGetProperty("snippets", out var snippetsEl) && snippetsEl.ValueKind != JsonValueKind.Null)
            {
                ...
                {
                    throw new ConfigException($"{sourcePath}: \"snippets\" must be an object of strings");
                }
    ...
    public static ParsedConfig Load(string cwd)
    {
    ...
    public static Value ToArtifact(ParsedConfig config) => Value.Map([
        new("docs", Value.Map([
    ...
    private static ImmutableArray<string> StringArray(JsonElement parent, string key, string label, string sourcePath)
    {
        ...
        {
            throw new ConfigException($"{sourcePath}: \"{label}\" must be an array of strings");
        }
