...
/// <summary>
/// The <c>doc.json</c> golden gate (T3): for every conformance bundle, parsing its
/// <c>example.md</c> and projecting with <see cref="Conformance.ToDocArtifact"/> must reproduce
/// the committed <c>golden/doc.json</c> byte-for-byte. Port of the Rust/Java var-doc gate.
/// </summary>
public class DocConformanceTests
{
    ...
    public void DocMatchesGolden(string bundle)
    {
        ...
        var actual = CanonicalJson.Stringify(Conformance.ToDocArtifact(doc));

        var expected = File.ReadAllText(Path.Combine(bundleDir, "golden", "doc.json"));
    ...
    public void EveryBundleIsWellFormed()
    {
        ...
        foreach (var dir in bundles)
        {
            ...
            foreach (var artifact in new[] { "doc", "registry", "plan", "trace" })
            {
                Assert.True(
