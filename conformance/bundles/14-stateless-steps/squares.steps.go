...
func Register(s *varar.Steps) {
	s.Stimulus("I warm up my mental math", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
	...
	s.Sensor("The square of {int} is {int}.", func(state varar.Value, args []varar.Value) varar.HandlerReturn {
		n, _ := args[0].AsInt()
		return varar.Returns(varar.ListValue(varar.IntValue(n), varar.IntValue(n*n)))
	})
}
