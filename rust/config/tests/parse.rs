...
use varar_config::{Config, parse_config, read_config};
...
fn read_returns_the_empty_config_when_there_is_no_file() {
    ...
    assert_eq!(read_config(&root).unwrap(), Config::default());
...
fn read_delegates_to_parse_and_labels_errors_with_the_path() {
    ...
    let err = read_config(&root).unwrap_err();
