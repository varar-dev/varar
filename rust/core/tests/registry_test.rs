...
fn define_parameter_type_makes_a_custom_type_available_to_subsequent_step_compilations() {
    let r = create_registry();
    let with_type = define_parameter_type(
        &r,
        "airport",
        "[A-Z]{3}",
        Rc::new(|g: &[&str]| Value::from(g[0])),
    );
    assert!(
        add_step(
            &with_type,
...
fn define_parameter_type_records_the_custom_type_immutably() {
    ...
    assert_eq!(
        vec![CustomParameterType::new("airport", "[A-Z]{3}")],
        r1.custom_parameter_types
    );
