"""test_plan.py — port of typescript/packages/core/tests/plan.test.ts."""
...
def _reg():
    r = create_registry()
    r = add_step(r, expression="I have {int} in my account", expression_source_file="steps.ts", expression_source_line=1, handler=_noop, kind="stimulus")
    r = add_step(r, expression="I withdraw {int}", expression_source_file="steps.ts", expression_source_line=2, handler=_noop, kind="stimulus")
    r = add_step(r, expression="I should have {int} left", expression_source_file="steps.ts", expression_source_line=3, handler=_noop, kind="stimulus")
    return r
...
def test_header_bound_table_expands_into_one_example_per_row() -> None:
    r = create_registry()
    r = add_step(r, expression="each row lists the dice, the category and the score", expression_source_file="s.ts", expression_source_line=1, handler=_noop, kind="stimulus")
    source = (
        "# Yahtzee\n\n"
...
def test_header_bound_matching_is_case_sensitive() -> None:
    r = create_registry()
    r = add_step(r, expression="each row lists the Dice and the Score", expression_source_file="s.ts", expression_source_line=1, handler=_noop, kind="stimulus")
...
def test_detached_table_produces_no_diagnostic() -> None:
    r = create_registry()
    r = add_step(r, expression="I have {int} cukes", expression_source_file="s.ts", expression_source_line=1, handler=_noop, kind="stimulus")
    source = (
        "# Detached\n\n"
        "Given I have 5 cukes.\n\n"
