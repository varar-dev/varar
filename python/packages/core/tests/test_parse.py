...
def test_parse_returns_doc_whose_examples_come_from_paragraphs_and_carry_heading_stack() -> None:
    source = "# Hello\n\nbody"
    doc = parse("hello.md", source)
    assert doc.path == "hello.md"
    assert doc.source == source
    # One paragraph → one Example. Scope from the heading above.
    assert len(doc.examples) == 1
    assert doc.examples[0].scope_stack == ("Hello",)
