...
def _golden(bundle: Path, name: str):
    """The golden's CONTENT. The comparison is deep equality of the parsed file,
    not of its bytes: the golden is one file generated by the TypeScript
    reference, and what every port must agree on is what it says — key order,
    indentation and escaping belong to whichever writer produced it."""
    return json.loads((bundle / "golden" / name).read_text(encoding="utf-8"))
...
def test_doc_matches_golden(bundle: Path) -> None:
    ...
    expected = _golden(bundle, "doc.json")
...
def test_registry_matches_golden(bundle: Path) -> None:
    ...
    assert artifact == _golden(bundle, "registry.json"), f"registry.json mismatch for {bundle.name}"
