...
description: Use when starting or reviewing a new Varar language port (Java, Kotlin, Go, ...) — porting the pure core, the runner shell, and a test-framework adapter behind the shared conformance suite. Symptoms: "add language X", "port Varar to X", deciding varar-core vs varar-runner vs adapter boundaries, or a conformance bundle not matching its goldens.
---
...
Varar is hexagonal + multi-language (ADR 0001). **TypeScript, Python, Java,
Kotlin, Ruby, Rust, C#, and Go are all complete ports at full parity** — there is no
...
to. Python,
Ruby and Go are the closest precedents for a *full pipeline* port (no runtime
sharing); Kotlin is the precedent for a *facade over an
...
`conformance/bundles/*/golden/*.json` — never by writing fresh
...
Goldens are compared **by content, not by bytes**: each port parses the golden
and asserts deep equality. The file is one artifact generated by the TypeScript
reference, so key order, indentation and escaping belong to whichever writer
produced it. (JSON *writers* are pinned only where a real file is produced —
`varar.lock.json` and `.varar/<oath>.json`.)

There are five conformance corpora, and they gate different things. Passing the
core ones and still shipping a broken port is possible — that is exactly what
happened in [#69][i69-intro]:

| Corpus | Gates | Shape |
|---|---|---|
| `conformance/bundles/` | the pure core: doc, registry, plan, trace | golden JSON, deep-equality |
...
| `conformance/run-results/` | the **wire format** of `.varar/<oath>.json` (ADR 0014) | one `expected.json`, built in each port's own types |
| `conformance/parity.json` | which core **capabilities** must exist in which ports | `release/check-parity.sh` / `make parity` |
...
- `doc/ARCHITECTURE.md` — the layer boundaries every port reproduces.
- The completed ports themselves. There are no per-port design documents: the
  code and the conformance corpora are the specification, which is why this
  skill points you at modules rather than at prose.
...
- A ported module's conformance golden doesn't match and you need to know
  which artifact stage (doc/registry/plan/trace) is at fault.
...
|---|---|---|---|
| `<lang>-core` (e.g. `varar-core` / `varar_core`) | nothing runtime-ish | pure pipeline: parse → match → plan → execute, diffs, drift, conformance projections | filesystem, network, globals, time, test-framework types |
| `<lang>` facade (e.g. `@varar/varar` / `varar`) | `<lang>-core` | author API only: `steps(factory)` / `Steps.state(factory)` (stimulus/sensor), `registry` glue subpath | pipeline internals directly (goes through core) |
| `<lang>-config` (e.g. `@varar/config` / `varar_config`) | nothing (pure) | the `varar.config.json` reader — strict, fail-loud; a pure `parse(text, source)` plus the thin `read(root)` over it; its own conformance corpus | filesystem beyond reading the one config file |
| `<lang>-runner` (e.g. `varar-runner`) | facade + config + core | imperative shell: oath/step discovery (globs), `load_steps`, `run_oath`/`plan_oath`, failure rendering, the filesystem `BaselineStore` (drift) | any one test framework's types |
| `<lang>-<framework>` (e.g. `@varar/vitest`, `pytest-varar`) | `<lang>-runner` | one test-framework binding: collection (one test item per example), fixture/DI bridging, reporting, the drift gate | pipeline logic (delegates to runner/core) |

`varar-config` is a **distinct shared package**, not folded into the runner — it
has its own conformance corpus at `conformance/config/cases/`
(see below). `varar-core`/`varar` were originally one package in Python and were
later split to mirror this shape exactly — **start a new
...
2. **Design spec doc(s)** — a short design note per sub-project.
...
3. **Task plan** — TDD task-by-task (write the translated unit test, watch it
   fail, port the module, gate on conformance, commit).
4. **Execute** one module at a time, gated by the four conformance artifacts
   in order (below) — never jump ahead to `trace.json` before `doc.json`
   is solid; each stage depends on the previous one being correct.

## Conformance-driven staging (the spine)

"Done" for the core = every bundle's four artifacts deep-equal the committed
goldens. Stage the work in this order, each an independently
gated milestone:

1. **`doc.json`** — parse only (scanner/structurer/inline → AST + spans).
...
2. **`registry.json`** — step registration (author API + cucumber-expression
   ...
   **Cucumber-expressions is a *library dependency*, not a hand-port.** Depend
   ...
   ports only Varar's own hit-resolution, ambiguity detection, and offset-shifting
...
## JSON: you need a reader, not a canonical writer

Goldens are compared **by content**. Your port parses `golden/*.json` into its
own value model and asserts deep equality against what the pipeline produced —
so key order, indentation, escaping and trailing newline are *not* your problem.
They belong to whichever writer generated the file (the TypeScript reference).

What this means concretely:

- **You need a JSON reader.** A language whose stdlib has none (Java, Rust,
  .NET, Go, per `conformance/parity.json`'s `json-reader` capability) ports a
  small one rather than pulling in Jackson/Gson/serde.
- **You need a value model** the golden and the artifact can both be projected
  into, plus a deep-equality function over it (`ValueEqual` in Go,
  the equivalent in each port).
- **Step-def files are referenced by stem, not extension** — `numerals.steps.ts`
  and `numerals.steps.py` both project as `"numerals.steps"`, which is what lets
  one golden serve every language's fixture for the same bundle. This *is* part
  of the contract.

**Where a byte-stable writer is still required:** the two files a port really
writes to disk — `varar.lock.json` (keys sorted, 2-space indent, trailing
newline, so a clean re-run makes no git diff) and `.varar/<oath>.json` (2-space
indent, trailing newline, declaration order — see ADR 0014). Those are pinned by
`conformance/run-results/` and by the drift tests, not by the bundle goldens.
...
Port these engine modules (TypeScript names in `varar-core/src/*.ts`; Python
...
list: files like `config.ts` and `find-files.ts` are `varar-config`/runner
...
Modeled on `@varar/vitest` (TS), `pytest-varar`/`varar-unittest` (Python), and
`varar-junit`/`varar-kotest` (JVM). **Each framework needs its own
...
- **Discovery/config**: one `varar.config.json` per workspace root, shared
  ...
  (`@varar/config` in TypeScript, `varar_config` in Python, `varar-config`
  in Java) — do not invent an ecosystem-idiomatic surface (no `[tool.varar]`
...
- **Drift gate**: each adapter reconciles every oath against `varar.lock.json`
  ...
  `pytest-varar`/`varar-unittest` `tests/test_drift.py`, `varar-kotest`'s
...
- **Pruning the baseline**: `reconcileDrift` is per-oath and never visits a path
  ...
  glob walk alone may be an incomplete view — `varar-junit` also resolves oaths as
...
- **Tree-sitter dialect** (the LSP/editor authoring surface — a *required*
  ...
  (`@varar/lsp/src/node-grammar-loader.ts`, `@varar/language/tests/test-grammar-loader.ts`),
  and the VS Code bundler's copy list (`@varar/vscode/esbuild.mjs`); add the grammar
  package to `@varar/language` (devDep) + `@varar/lsp` (dep) + both `knip.json`
...
The **`language-coverage.test.ts` drift gate** (@varar/language) enforces the last
...
## Config conformance corpus (a distinct gate)

`varar-config` has its own corpus at `conformance/config/cases/*/`, separate from
...
- A CLI (`@varar/cli`) — TS-only so far, not part of the core/runner/adapter
...
| What are the layer boundaries? | `doc/ARCHITECTURE.md` |
| Which capabilities must my port have, and where do they live? | `conformance/parity.json` |
| How does the runner + test-framework adapter fit together? | `python/packages/runner/` + `python/packages/pytest/`, or `go/runner/` |
...
| The conformance corpus + goldens | `conformance/bundles/*/{example.md, *.steps.{ts,py,kt,rb,rs,cs,go}, *Steps.java, golden/*.json}` — 19 bundles, four artifacts each; **plus** the config corpus `conformance/config/cases/*/{varar.config.json, golden.json|expect-error.txt}` |
...
- **Leaking engine imports into the facade or vice versa.** `<lang>-core`
  ...
  way the Python split plan did (`grep -rn "from varar\b" <core-src>` → empty).
...
- A new language that **shares a runtime with an existing port** can be a thin
  **facade over that port's engine** — no second pipeline, no full four-artifact
  conformance run. **Kotlin did exactly this over Java**: `varar-kotlin`
  (`dev.varar.kotlin`) is an author-facade + Kotest adapter sitting on the
  compiled Java `varar-core`; both are JVM bytecode. Its conformance scope is the
