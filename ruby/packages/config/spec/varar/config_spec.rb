...
module Varar
  # read_config is the filesystem edge over parse_config (issue #11).
  ...
  ::RSpec.describe Config do
    describe '.parse_config' do
      it 'reads every key without touching the filesystem' do
        cfg = described_class.parse_config(
      ...
      it 'labels errors with the given source' do
        expect { described_class.parse_config('{oops', 'buffer://untitled') }
          .to raise_error(ArgumentError, %r{buffer://untitled})
      end

      it 'parses an empty object as the empty config' do
        expect(described_class.parse_config('{}', '<memory>')).to eq(Config::Config.new)
      end
    end

    describe '.read_config' do
      it 'returns the empty config when there is no file' do
        Dir.mktmpdir { |tmp| expect(described_class.read_config(tmp)).to eq(Config::Config.new) }
      end

      it 'delegates to parse_config, labelling errors with the path' do
        Dir.mktmpdir do |tmp|
          File.write(File.join(tmp, 'varar.config.json'), '{oops')
          expect { described_class.read_config(tmp) }
