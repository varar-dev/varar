...
func registerTablesAndDocstrings(s *varar.Steps) {
	...
	s.Sensor("Uppercase each one:", func(state varar.Value, table varar.Value) (varar.Value, error) {
		out := []varar.Value{}
	...
	s.Sensor("Greet {word}:", func(state varar.Value, name, doc string) (string, string, error) {
