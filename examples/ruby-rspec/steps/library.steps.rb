...
param.call('date', '[A-Z][a-z]+ \d{1,2}, \d{4}', parse: to_date, format: format_date)
# The amount is cucumber-expressions' float regexp, minus scientific notation.
param.call('money', '£(?=.*\d.*)[-+]?\d*(?:\.(?=\d.*))?\d*|\d+p', parse: to_money, format: format_money)
...
param.call('title', '\*[^*]+\*', parse: ->(raw) { raw[1...-1] }, format: ->(t) { "*#{t}*" })

stimulus.call('borrowed {title}, due back on {date}') do |state, title, due|
  { loans: state[:loans] + [{ title: title, due: due }] }
end

stimulus.call('returns it on {date}') do |state, returned_on|
...
sensor.call('owes a {money} late fee') { |state, _expected| state[:fee] }

sensor.call('{money} for each day overdue') { |_state, _expected| Library::FEE_PER_DAY }

stimulus.call('asks to borrow {title} on {date}') do |state, _title, on|
  { granted: Library.may_borrow(state[:loans], on) }
end

sensor.call('the library refuses') { |state| raise 'expected the library to refuse' if state[:granted] }

sensor.call('the library agrees') { |state| raise 'expected the library to agree' unless state[:granted] }
