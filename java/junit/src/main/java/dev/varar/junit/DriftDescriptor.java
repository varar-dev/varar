 ...
 * {@code -Dvarar.update} / {@code VARAR_UPDATE=1}). Added by {@link OathFileSelectorResolver} alongside
 * the file's example leaves.
 *
 * <p>{@code UniqueId} segment type {@link #SEGMENT_TYPE} ({@code "drift"}), value {@code
 * "drift-<line>"} — deliberately distinct from {@link ExampleDescriptor}'s bare-line values so
 * it never collides with, or is mistaken for, an example leaf.
 */
final class DriftDescriptor extends AbstractTestDescriptor implements Node<OathEngineExecutionContext> {

    static final String SEGMENT_TYPE = "drift";

    private final String message;

    DriftDescriptor(UniqueId uniqueId, String displayName, TestSource source, String message) {
    ...
    public OathEngineExecutionContext execute(
            OathEngineExecutionContext context, DynamicTestExecutor dynamicTestExecutor) {
