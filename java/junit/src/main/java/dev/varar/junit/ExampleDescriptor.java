...
/**
 * One leaf per {@link Plan.PlannedExample}, a child of the {@link OathFileDescriptor} its example
 * was planned from.
 *
 ...
 * OathFileDescriptor} rather than duplicated here.
 *
 * <h2>Execution (Task 11)</h2>
 *
 * <p>{@link #execute} looks up ITS OWN {@link dev.varar.runner.Run.ExampleRun} from the
 * parent {@link OathFileDescriptor}'s cache ({@link OathFileDescriptor#runFor}) rather than
 ...
 */
final class ExampleDescriptor extends AbstractTestDescriptor implements Node<OathEngineExecutionContext> {

    static final String SEGMENT_TYPE = "example";

    private final Plan.PlannedExample example;

    ExampleDescriptor(UniqueId uniqueId, String displayName, TestSource source, Plan.PlannedExample example) {
    ...
    @Override
    public OathEngineExecutionContext execute(
    ...
    /**
     * This leaf's parent, always a {@link OathFileDescriptor} — every {@link
     * ExampleDescriptor} is created and immediately added as a child of exactly one, by {@link
     * OathFileSelectorResolver#createDescriptor}, so by execution time (long after discovery)
     * {@link #getParent()} is populated.
     */
    private OathFileDescriptor fileDescriptor() {
        return (OathFileDescriptor)
