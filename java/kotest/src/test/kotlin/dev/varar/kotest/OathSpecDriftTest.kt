...
/**
 * An oath whose paragraph the committed baseline recorded as an example but which now matches no
 * step. Driven ONLY by [OathSpecDriftTest] (name doesn't match Surefire's *Test includes, so it
 * never fails the module's own build). The committed `kotest-drift/varar.lock.json` is preserved on
 * an unacknowledged drift (never rewritten), so this fixture is stable across runs.
 */
class DriftOathSpec : OathSpec(root = Path.of("src/test/resources/kotest-drift"))

class OathSpecDriftTest {

    @Test
    fun `a drifted paragraph surfaces as a failing Kotest test`() {
        val results =
            EngineTestKit.engine("kotest")
                .selectors(selectClass(DriftOathSpec::class.java))
                .execute()
