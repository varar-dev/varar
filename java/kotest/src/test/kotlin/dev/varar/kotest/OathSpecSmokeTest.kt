 ...
 * still fails the build (proven by OathSpecFailureTest).
 *
 ...
 */
class OathSpecSmokeTest : OathSpec(root = Path.of("src/test/resources/kotest-smoke"))

/**
 * Guards against a silent zero-tests-discovered pass.
 *
 * Accessor note: the brief's `rootTests()` does not exist on Kotest 6.2.1's
 * `io.kotest.core.spec.Spec` — resolved empirically via `javap`/sources-jar on the resolved
 * `kotest-framework-engine-jvm:6.2.1` artifact. `Spec` declares `tests(): List<TestDefinition>`
 * (implemented by `DslDrivenSpec`, the base FunSpec inherits from); every root-level
 * `context(...)`/`test(...)` call registers exactly one `TestDefinition` via `add(...)` (see
 ...
 */
class OathSpecRegistrationTest :
    FunSpec({
        test("OathSpec registered one container with one example") {
            val spec = object : OathSpec(root = Path.of("src/test/resources/kotest-smoke")) {}
