...
/**
 * Deliberately-failing spec driven ONLY programmatically by [OathSpecFailureTest] via EngineTestKit
 * — its name must not match Surefire's default *Test includes, or the red example would fail the
 * module's own build. Reuses the smoke fixture's steps: the sensor returns 5 while oaths/wrong.md
 * claims 99. Config comes from `src/test/resources/kotest-failing/varar.config.json`.
 */
class FailingOathSpec : OathSpec(root = Path.of("src/test/resources/kotest-failing"))
    ...
    fun `a failing example surfaces through the Kotest engine with the rendered failure`() {
        val results =
            EngineTestKit.engine("kotest")
                .selectors(selectClass(FailingOathSpec::class.java))
                .execute()
