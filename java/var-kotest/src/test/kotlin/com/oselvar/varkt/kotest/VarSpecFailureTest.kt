...
/**
 * Deliberately-failing spec driven ONLY programmatically by
...
/**
 * Covers VarSpec's only nontrivial logic: the failing-example path, where the
 * thrown failure is wrapped in an AssertionError carrying var-runner's
 * span-anchored rendering (Render.renderFailure) with the original failure as
 * cause. Also the empirical proof that a failing var example fails a Kotest
 * run (Surefire's console count for Kotest specs reads "Tests run: 0", so a
 * green build alone would not prove this).
 */
class VarSpecFailureTest {

    @Test
    fun `a failing example surfaces through the Kotest engine with the rendered failure`() {
        val results = EngineTestKit.engine("kotest")
        ...
        assertEquals(1, results.testEvents().failed().count(), "expected exactly one failed example")
        results.testEvents().assertThatEvents().haveExactly(
            1,
            event(
                test(),
                finishedWithFailure(
